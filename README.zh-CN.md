...
![macOS 输入源切换与按文稿记忆设置](docs/images/macos-input-source-switching-and-document-memory.png)

**图 4：** 在“所有输入法”中保留 ABC 和简体拼音，打开菜单栏输入法菜单，并启用
“使用中/英键切换 ABC”。部分键盘或 macOS 版本会把这里显示为 Caps Lock；两者都
表示使用键盘上的专用中英文切换键。图中同时开启了“自动切换到文稿的输入法”。
...
![macOS 简体拼音的半角标点设置](docs/images/macos-pinyin-halfwidth-punctuation.png)

**图 5：** 在左侧选择“简体拼音”，然后启用“使用半角标点符号”。中文输入状态下的
逗号、句号、括号等会尽可能使用适合代码、Markdown、命令和文件名的半角形式。
